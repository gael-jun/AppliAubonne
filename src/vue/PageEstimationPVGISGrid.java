...
public class PageEstimationPVGISGrid extends JPanel {
    ...
    /** Panel d'affichage des graphes (unifié avec OffGrid). */
    ...
    // Bouton PDF (même si remplacé par le menu Export, il sert à initialiser le champ final et garder la parité Off-Grid)
    ...
    // Bouton Export (menu déroulant) comme Off-Grid
    ...
    // Bouton Finances (apparaît après réception des données)
    ...
    // Harmonise la hauteur de l'icône du bouton Finances avec celle des autres boutons
    try {
        if (finIcon != null) {
    ...
    // Remplace le bouton PDF direct par un menu Export
    ...
    // Augmente un peu l'espace réservé au formulaire (à gauche)
    split.setResizeWeight(0.32);
    split.setDividerLocation(0.32);
    split.setOneTouchExpandable(true);
    add(split, BorderLayout.CENTER);

    // Barre de statut en bas (gris clair)
     ...
     * Aligne le comportement sur la page Off-Grid.
     */
    private void exporterResultatsEnCSV() {
        ...
        try {
            ...
            // Construire un PVGISResult minimal à partir du JSON (déjà implémenté pour finances)
            modele.PVGISResult pvr = buildPVGISResultFromGridJson();

            // Préparer l'objet FinancialResult si des données financières sont présentes
    ...
    private void exporterResultatsEnPDF() {
        ...
        try {
            ...
            try {
                ...
                float y = 750;
        // Titre + logo
                String titleText = "Estimation PVGIS Grid-Connected";
                ...
                try {
                    ...
                    java.io.InputStream is = PageEstimationPVGISGrid.class.getResourceAsStream("/ressources/logo.png");
                    if (is != null) {
    ...
    // Anciennes méthodes utilitaires supprimées (remplacées par GraphsPanel et util.ChartFactory)
